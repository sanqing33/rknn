...
int main(int argc, char **argv)
{
	...
	while(cap.read(frame)) {
        ...
        for (int i = 0; i < od_results.count; i++)
        {
            ...
            sprintf(text, "%s %.1f%%", cls_to_name(det_result->cls_id), det_result->prop * 100);
            putText(frame, text, cv::Point(x1, y1 - 6), cv::FONT_HERSHEY_DUPLEX, 0.7, cv::Scalar(0,0,255), 1, cv::LINE_AA);
        }

		// 计算FPS
        timer0.tok();
        timer0.print_time("once run and process");
		cv::imshow("TACO YOLOv8_Seg Demo", frame);
