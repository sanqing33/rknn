...
int inference_ppocr_cls_model(rknn_app_context_t* app_ctx, const cv::Mat& srcimage, ppocr_cls_result* out_result)
{
    int ret;
    rknn_input inputs[1];
    rknn_output outputs[1];
    ...
    cv::resize(image, image, cv::Size(resized_w, imgH));
    image.convertTo(image, CV_32FC3);
    image = (image - 127.5)/127.5;
    if (resized_w < imgW) {
        copyMakeBorder(image, image, 0, 0, 0, imgW- resized_w, cv::BORDER_CONSTANT, 0);
    }
    ...
    inputs[0].size  = app_ctx->model_width * app_ctx->model_height * app_ctx->model_channel * sizeof(float);
    inputs[0].buf = malloc(inputs[0].size);
    memcpy(inputs[0].buf, image.data, inputs[0].size);
    ...
    // PostProcess
    output = (float*)outputs[0].buf;
    if(output[1] > output[0]){
        out_result->cls_score = output[1];
        out_result->cls_label = 1;
    } else {
        out_result->cls_score = output[0];
        out_result->cls_label = 0;
    }
...
int inference_ppocr_system_model(ppocr_system_app_context* sys_app_ctx, image_buffer_t* src_img, ppocr_det_postprocess_params* params, ppocr_text_recog_array_result_t* out_result)
{
    ...
    for (int i=0; i < boxes_result.size(); i++) {
        ...
        // cls
        if(sys_app_ctx->cls_context.status == 1 ){
            ppocr_cls_result cls_result;
            inference_ppocr_cls_model(&sys_app_ctx->cls_context, crop_image, &cls_result);
            // printf("cls_score: %f cls_label: %d\n",cls_result.cls_score, cls_result.cls_label);
            if (cls_result.cls_label % 2 == 1 && cls_result.cls_score > CLS_THRESH) {
                cv::rotate(crop_image, crop_image, 1);
            }
        }
