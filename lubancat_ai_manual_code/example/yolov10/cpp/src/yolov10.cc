...
int inference_yolov10_model(rknn_app_context_t *app_ctx, image_buffer_t *img, object_detect_result_list *od_results)
{
    ...
    printf("convert_image_with_letterbox use %f ms\n", (__get_us(stop_time) - __get_us(start_time)) / 1000);
#endif
...
#if defined(TIMEVAL_OUTPUT)
    gettimeofday(&stop_time, NULL);
    printf("post_process use %f ms\n", (__get_us(stop_time) - __get_us(start_time)) / 1000);
#endif

    // Remeber to release rknn output
    // printf("rknn_outputs_release\n");
